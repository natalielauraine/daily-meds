...
// Appears when the user taps the "Add to playlist" button on the session page.
...
export default function AddToPlaylistModal({ sessionId, onClose }: Props) {
  ...
  // Load playlists fresh every time the modal opens
  useEffect(() => {
  ...
  function handleCreate() {
    ...
    addToPlaylist(playlist.id, sessionId);
    setPlaylists(getPlaylists());
    setNewName("");
    setCreating(false);
  }
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col gap-2 mb-4 max-h-64 overflow-y-auto">
          ...
          {playlists.map((p) => {
            const included = isInPlaylist(p.id, sessionId);
            return (
              <button
                key={p.id}
                onClick={() => handleToggle(p.id)}
                className="flex items-center gap-3 w-full px-4 py-3 rounded-[10px] text-left transition-colors hover:bg-white/[0.04]"
