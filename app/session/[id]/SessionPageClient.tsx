    ...
    <div className="relative min-h-screen" style={{ backgroundColor: "#010101", color: "white" }}>
      ...
      <section className="relative w-full overflow-hidden" style={{ height: "70svh", minHeight: "450px" }}>
        ...
        <div className="absolute left-6 md:left-12 z-10 max-w-2xl" style={{ bottom: "220px" }}>
          ...
          {/* Inline actions: favorite + notes + share */}
          <div className="flex items-center gap-3">
            {isLoggedIn && (
              <button
                onClick={handleSave}
                className="flex items-center gap-2 px-3 py-1.5 rounded-full text-[11px] font-bold uppercase tracking-wide transition-all"
                style={{
                  background: savedInApp ? "rgba(173,242,37,0.15)" : "rgba(255,255,255,0.08)",
                  border: `1px solid ${savedInApp ? "rgba(173,242,37,0.3)" : "rgba(255,255,255,0.12)"}`,
                  color: savedInApp ? "#ADF225" : "rgba(255,255,255,0.5)",
                }}
              >
                <svg width="12" height="12" viewBox="0 0 24 24" fill={savedInApp ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2">
                  <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
                </svg>
                {savedInApp ? "Favorited" : "Favorite"}
              </button>
            )}
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 rounded-full text-[11px] font-bold uppercase tracking-wide transition-all"
              style={{
                background: showNotes ? "rgba(255,65,179,0.15)" : "rgba(255,255,255,0.08)",
                border: `1px solid ${showNotes ? "rgba(255,65,179,0.3)" : "rgba(255,255,255,0.12)"}`,
                color: showNotes ? "#ff41b3" : "rgba(255,255,255,0.5)",
              }}
            >
      ...
      {/* ── EXPERIENCE NOTES SIDEBAR ── */}
      <div
        className="fixed top-0 right-0 z-50 h-full transition-transform duration-300"
        style={{
      ...
      {showShareModal && (
        <ShareSessionModal
          session={{ id: session.id, shortId: session.shortId, title: session.title, moodCategory: session.moodCategory, duration: session.duration, gradient: session.gradient, thumbnail: session.thumbnail }}
